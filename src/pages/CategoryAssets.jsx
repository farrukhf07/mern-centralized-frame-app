...
const CategoryAssets = () => {
  ...
  const fetchAssets = async () => {
    try {
      setLoading(true);
      setError(null);
      const response = await axiosInstance.get(API_ENDPOINTS.GET_ASSETS_OF_CATEGORY(categoryId));
  ...
  const handleEditAsset = (asset) => {
    // Navigate to edit page with placeholder appId as it's from central data
    ...
    <div className="page-wrapper fade-in">
      <div className="container-fluid">
        ...
        <div className="page-header">
          ...
          <p className="subtitle">Manage assets within this category</p>
        </div>
        ...
        <div className="action-bar">
          ...
          />
          <button className="btn btn-accent" onClick={() => navigate(`/create-asset/${categoryId}`)}>
            ➕ Create New Asset
          </button>
        </div>
        ...
        {filteredAssets.length === 0 && !error && (
          <div className="empty-state">
            ...
            <p>{assets.length > 0 ? "No matching assets found." : "No assets found in this category."}</p>
          </div>
        )}
        <AssetPreviewModal
