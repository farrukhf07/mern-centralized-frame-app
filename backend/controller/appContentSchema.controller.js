...
// exports.addAssetToConfig = async (req, res) => {
//     try {
//         const { appId, categoryId, assetId } = req.body;

//         // Verify Asset exists in master table
//         const assetExists = await Asset.exists({ _id: assetId });
//         if (!assetExists) return res.status(404).json({ message: "Asset not found in master" });

//         const config = await AppConfig.findOneAndUpdate(
//             { appId, "selections.categoryId": categoryId },
//             { $addToSet: { "selections.$.assets": assetId } },
//             { new: true }
//         );

//         if (!config) return res.status(404).json({ message: "Config or Category mapping not found" });
...
// exports.removeAssetFromConfig = async (req, res) => {
//     try {
//         const { appId, categoryId, assetId } = req.body;

//         const config = await AppConfig.findOneAndUpdate(
//             { appId, "selections.categoryId": categoryId },
//             { $pull: { "selections.$.assets": assetId } },
//             { new: true }
//         );

//         res.status(200).json({ success: true, message: "Asset removed from app view", config });
//     } catch (error) {
//         res.status(500).json({ success: false, message: error.message });
//     }
// };
...
// exports.removeCategoryFromConfig = async (req, res) => {
//     try {
//         const { appId, categoryId } = req.body;

//         const config = await AppConfig.findOneAndUpdate(
//             { appId },
//             { $pull: { selections: { categoryId: categoryId } } },
//             { new: true }
//         );

//         res.status(200).json({ success: true, message: "Category removed from app view", config });
//     } catch (error) {
//         res.status(500).json({ success: false, message: error.message });
//     }
// };
...
// exports.createCategoryAndLink = async (req, res) => {
//     try {
//         const { appId } = req.params; // Or req.body.appId
//         const { name, isEnable, isPremium, sequence, customFields } = req.body;

//         if (!appId) {
//             return res.status(400).json({ success: false, message: "App ID is required to link the category" });
//         }

//         // 1. Handle Image Processing (Same as your original category controller)
...
//         // 2. Create the Category in the master Category table
...
//         // 3. Link this new category to the AppConfig table
//         // We use $push to add a new object to the selections array
...
//         res.status(201).json({
//             success: true,
//             message: "Category created in master and linked to App Config",
...
// exports.createAssetAndLink = async (req, res) => {
//     try {
//         const { appId, categoryId } = req.body;       
//         if (!appId || !categoryId) {
//             return res.status(400).json({ success: false, message: "appId and categoryId are required" });
//         }

//         // 1. Image Processing Logic (Adapted from your asset.controller.js)
//         const imageFile = req.files?.image?.[0]; // Assuming single upload for "Create New"
//         if (!imageFile) {
...
//         // Target directories (using category name for folder organization)
...
//         const finalFilename = `${Date.now()}_${baseName}${originalExt}`;
//         const targetImagePath = path.join(imageDir, finalFilename);

//         // Move image from temp to final
//         fs.renameSync(imageFile.path, targetImagePath);

//         // Generate thumbnail
//         const metadata = await sharp(targetImagePath).metadata();
//         const thumbWidth = Math.floor(metadata.width / 3);
//         const thumbHeight = Math.floor(metadata.height / 3);
...
//         // 2. Create the Asset in the master table
//         const assetData = {
//             name: baseName,
...
//         const newMasterAsset = await Asset.create(assetData);

//         // 3. Link the Asset to the specific AppConfig
//         // We find the config for the appId and the specific category selection, then push the asset ID
//         const updatedConfig = await AppConfig.findOneAndUpdate(
//             { 
//                 appId: appId, 
...
//             },
//             { 
//                 $push: { "selections.$.assets": newMasterAsset._id } 
//             },
//             { new: true }
//         );
...
//                 success: false, 
//                 message: "App configuration or Category mapping not found. Please link the category to the app first." 
//             });
//         }

//         res.status(201).json({
//             success: true,
//             message: "Asset created and linked to App successfully",
//             asset: newMasterAsset,
//             config: updatedConfig
//         });

//     } catch (error) {
//         console.error("Error in createAssetAndLink:", error);
//         res.status(500).json({ success: false, message: error.message });
//     }
// };
...
const hardDeleteAppContentByCategory = async (categoryId) => {
...
exports.hardDeleteAppContentByCategory = hardDeleteAppContentByCategory;